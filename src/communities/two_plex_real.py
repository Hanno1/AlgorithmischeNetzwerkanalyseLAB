...
def search_2_plex_version(G_: Graph, version=4):
    G = G_.copy_graph()
    l = 0
    max_2_plex = set()
    # first improvement
    ...
    for node in deg_sorting:
        ...
        res = _search_two_plex_version_rek(copy.deepcopy(new_G), set(), max_2_plex, version)
        if res:
            max_2_plex = res
            l = len(max_2_plex)
        G.remove_node(node)
...
def _search_two_plex_version_rek(G, F, max_2_plex, version):
    # second Improvement
    if version > 2:
    ...
    if v not in F:
        new_G = G.copy_graph()
        new_G.remove_node(v)
        res = _search_two_plex_version_rek(new_G, copy.deepcopy(F), max_2_plex, version)
        if res:
            max_2_plex = res
    ...
    if len(not_neighbors & F) < 1:
        for u in not_neighbors:
            ...
            for nn in not_neighbors:
                if nn != u:
                    G.remove_node(nn)
            res = _search_two_plex_version_rek(G, F, max_2_plex, version)
            if res:
                max_2_plex = res
            break
    return max_2_plex
...
def search_2_plex_main(G_: Graph):
    G = G_.copy_graph()
    # first improvement
    max_2_plex = greedy_plex(G)
    l = len(max_2_plex)
    ...
    for node in deg_sorting:
        ...
        # start recursion algorithm for the new Graph
        res = search_two_plex_rek(copy.deepcopy(new_G), set(), max_2_plex)
        if res:
            max_2_plex = res
            l = len(max_2_plex)
        G.remove_node(node)
...
def search_two_plex_rek(G, F, max_2_plex):
    # second Improvement
    if len(G.get_nodes()) <= len(max_2_plex):
        return None
    ...
    if v not in F:
        new_G = G.copy_graph()
        new_G.remove_node(v)
        res = search_two_plex_rek(new_G, copy.deepcopy(F), max_2_plex)
        if res:
            max_2_plex = res
    ...
    if len(not_neighbors & F) < 1:
        for u in not_neighbors:
            ...
            for nn in not_neighbors:
                if nn != u:
                    G.remove_node(nn)
            res = search_two_plex_rek(G, F, max_2_plex)
            if res:
                max_2_plex = res
            break
    return max_2_plex
